...
list_page() {
  ...
  while true; do
    ...
    (( total == 0 )) && return
    local page_count=$(( (total + per_page - 1) / per_page ))
    (( page_count < 1 )) && page_count=1
    (( page > page_count )) && page=$page_count
    (( page < 1 )) && page=1
    local start_row=$(( (page - 1) * per_page + 1 ))
    local end_row=$(( page * per_page ))
    (( end_row > total )) && end_row=$total
    local previous_count=$((start_row - 1))
    ...
    if picker_run "⌕  Search "; then
      case "$PICKER_RESULT_SLUG" in
        __page_previous__) page=$((page - 1)) ;;
        __page_next__)     page=$((page + 1)) ;;
        __page_back__)     return ;;
        '') ;;
