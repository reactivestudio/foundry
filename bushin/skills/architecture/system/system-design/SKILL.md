...
1. **Findings** — concrete gaps. NFRs without numbers; missing arithmetic; named bottlenecks; sync chains; **at-least-once delivery (queue/Kafka) terminating in a state mutation (DB write, ZADD, counter) without an idempotency check**; **shard-key skew** (N% of traffic on M% of keys when N≫M; dedicated pools mask but don't fix it).
...
- Strong consistency by default; "strong + multi-region + low latency" together (CAP — pick two).
- Cache strategy (write-through / write-back / TTL) applied blanket without per-data-type staleness budget — invalidation *becomes* the system.
