...
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-new.sh:*) Read
---
...
2. **Scaffold.** `Bash`: `${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-new.sh "<title>" [<slug>]`. On exit 1 (name collision / invalid) — relay diagnostic and stop. On exit 3 (template missing) — ask user to run `/setup` first.
...
- If `.spec/changes/_template/` is missing in the project root and `$CLAUDE_PLUGIN_ROOT` is not exported, `change-new.sh` fails with a clear message — run `/setup` to bootstrap.
