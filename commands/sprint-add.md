...
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-locate.sh:*) Bash(${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-move.sh:*) Read
---
...
1. **Locate.** `Bash`: `${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-locate.sh <name>`. Exit 1 → "change not found"; exit 2 → "ambiguous (multiple buckets)".
...
3. **Move.** `Bash`: `${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-move.sh <name> sprint user`. `change-move.sh` appends `{ _meta, moved-to-sprint, by: user }` history entry automatically.
