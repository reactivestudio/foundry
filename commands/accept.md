...
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-locate.sh:*) Bash(${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-move.sh:*) Bash(${CLAUDE_PLUGIN_ROOT}/scripts/spec/tracking-get-stage.sh:*) Read AskUserQuestion
---
...
3. **Check stages.** `Bash`: two calls, `tracking-get-stage.sh <abs-path> implementation` and `… verification`.
...
4. **Move.** `Bash`: `${CLAUDE_PLUGIN_ROOT}/scripts/spec/change-move.sh <name> done user`.
