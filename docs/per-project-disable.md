...
Claude Code supports per-project plugin enable/disable natively. This document captures the operational details for `bushin`.
...
Claude Code writes the disable flag into the project's settings. Re-enable with `/plugin enable bushin@reactivestudio`.
...
```json
{ "disabledPlugins": ["bushin@reactivestudio"] }
```
or
```json
{ "plugins": { "bushin@reactivestudio": { "enabled": false } } }
```
