...
A change moves through 4 directories — `backlog/` → `in-progress/` → `done/` (or `→ declined/`). Each change has 6 stages (`refinement`, `design`, `decomposition`, `implementation`, `verification`, `termination`), each with its own state from an 8-element set (`estimation | required | skipped | pending | in-progress | review | completed | rejected`). Top-level `status:` and `stage:` fields are derived from the stages. Spec-commands are **state API only** — they don't generate content (agents do).

| Command | Purpose |
|---|---|
| `/change` | Read-only browse + drill + scaffold. Bare → tabbed list (`All` / `backlog` / `in-progress` / `closed`) with quartile-circle progress per change. `/change <bucket>` → switch tab. `/change <slug>` → drill into a change with context-aware action menu (state mutations: start / send to review / approve / reject / skip / decline / set scope). `/change "<task text>"` → LLM-generate slug + title + description, scaffold in backlog, write task text to `propose.md`, offer to launch `system-analyst` for refinement. |
| `/workflow <name>` | Orchestrator that drives a change end-to-end through all 6 stages. Reads active stage + state, delegates production to the matching producer agent (system-analyst / architect / teamlead / code-implementor / qa-engineer / termination-handler) via Task tool, gates advancement on AskUserQuestion approve/rework/reject/pause. Implementation stage iterates `roadmap.sh ready` tasks via code-implementor (1 task per invocation). |

Two user-facing slash commands for `.spec/changes/` besides `/setup`: `/change` (state inspection + manual mutations) and `/workflow` (automated stage progression with approve-loop). Auto-move: a change auto-flips to `in-progress/` when implementation leaves `{estimation, required}`, and to `done/` when all three terminal stages (implementation, verification, termination) reach `completed` or `skipped`. The `0.13.0` line ships the orchestrator + 4 new producer agents (`architect`, `teamlead`, `qa-engineer`, `termination-handler`) and their stage skills (`spec-design`, `spec-decomposition`, `spec-verification`, `spec-termination`, plus the paradigm doc `spec-workflow`). The `0.9.0` line was a breaking change from 0.8.x: state set rewritten (added `estimation`, `required`, `rejected`; renamed `need-approve→review`, `approved→completed`; `pause` folded into `pending`).
