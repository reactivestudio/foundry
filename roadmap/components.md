...
файлы копируются как есть, пустые каталоги (`projects/`, `cache/`,
`observability/metrics/`) создаются на месте. Скелет — единственное место, где
раскладка задана исполнимо; текст ниже её объясняет, но не дублирует. Код
`setup` на неё ещё не переведён — он едет вместе с переездом стора.
...
│   └── change/                      # скелеты proposal.md и tracking.yaml
...
У `03-design` в `output/` рядом лежит `validation-contract.md` (без него state
machine стадию не закроет), у `05-plan` — по файлу на задачу. Форма папки
стадии одна на все стадии, наполнение `output/` своё.
...
- **Пайплайн разворачивается один раз.** `templates/pipelines/<type>.yaml`
  ...
  переписывает задним числом то, что уже в работе.
